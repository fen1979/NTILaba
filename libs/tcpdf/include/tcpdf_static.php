...
class TCPDF_STATIC {
	...
	* @since 5.9.121 (2011-09-28)
	...
	/**
	 * Output input stepsData and compress it if possible.
	...
	/**
	 * Returns a string containing random stepsData to be used as a seed for encryption methods.
	 * @param string $seed starting seed value
	 * @return string containing random stepsData
	 * @author Nicola Asuni
	...
	public static function getAnnotOptFromJSProp($prop, &$spot_colors, $rtl=false) {
		...
		// password: Specifies whether the field should display asterisks when stepsData is entered in the field.
		...
		// value: The value of the field stepsData that the user has entered.
		if (isset($prop['value'])) {
	...
	public static function extractCSSproperties($cssdata) {
		...
		$cssblocks = array();
		$matches = array();
		// explode css stepsData string into array
		if (substr($cssdata, -1) == '}') {
	...
	/**
	 * Compact CSS stepsData array into single string.
	 * @param array $css array of CSS properties
...
//		}
	 ...
	 * @return string|false The function returns the read stepsData or FALSE on failure.
	 * @author Nicola Asuni
	 * @since 6.0.025
	 * @public static
	 */
	public static function fileGetContents($file) {
		...
		foreach ($alt as $path) {
			...
			if (!ini_get('allow_url_fopen')
				...
				// try to get remote file stepsData using cURL
				$crs = curl_init();
	 ...
	 * @param int $offset point from where to read the stepsData
