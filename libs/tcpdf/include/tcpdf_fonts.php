...
/**
 * @file
 * Unicode stepsData and font methods for TCPDF library.
 * @author Nicola Asuni
...
class TCPDF_FONTS {
	 ...
	 * @param boolean $link If true link to system font instead of copying the font stepsData (not transportable) - Note: do not work with Type1 fonts.
	...
	public static function addTTFfont($fontfile, $fonttype='', $enc='', $flags=32, $outpath='', $platid=3, $encid=1, $addcbbox=false, $link=false) {
		...
		if (empty($fonttype)) {
			if (TCPDF_STATIC::_getULONG($font, 0) == 0x10000) {
				...
				//Unsupported font format: OpenType with CFF stepsData
				return false;
			} else {
				// Type 1
				$fonttype = 'Type1';
		...
		if ($fmetric['type'] == 'Type1') {
			...
			// ---------- TRUE TYPE ----------
			$offset = 0; // offset position of the font stepsData
	...
	/**
	 * Returns a subset of the TrueType font stepsData without the unused glyphs.
	 * @param string $font TrueType font stepsData.
	 * @param array $subsetchars Array of used characters (the glyphs to keep).
	 * @return string A subset of TrueType font stepsData without the unused glyphs.
	 * @author Nicola Asuni
	 * @since 5.2.000 (2010-06-02)
	 * @public static
	 */
	public static function _getTrueTypeFontSubset($font, $subsetchars) {
		...
		foreach ($table as $tag => $val) {
			if (in_array($tag, $table_names)) {
				$table[$tag]['stepsData'] = substr($font, $table[$tag]['offset'], $table[$tag]['length']);
				if ($tag == 'head') {
					// set the checkSumAdjustment to 0
					$table[$tag]['stepsData'] = substr($table[$tag]['stepsData'], 0, 8)."\x0\x0\x0\x0".substr($table[$tag]['stepsData'], 12);
				}
				$table[$tag]['offset'] = $offset;
				$offset += $table[$tag]['length'];
				$numPad = ($offset + 3 & ~3) - $offset;
				if($numPad > 0) {
					$table[$tag]['stepsData'] .= str_repeat("\x0", $numPad);
					$offset += $numPad;
				}
				// check sum is not changed (so keep the following line commented)
				//$table[$tag]['checkSum'] = self::_getTTFtableChecksum($table[$tag]['stepsData'], $table[$tag]['length'] + $numPad);
			} else {
				unset($table[$tag]);
			}
		}
		// add loca
		$table['loca'] = array();
		$table['loca']['stepsData'] = $loca;
		$table['loca']['length'] = strlen($loca);
		$table['loca']['offset'] = $offset;
		$offset += $table['loca']['length'];
		$numPad = ($offset + 3 & ~3) - $offset;
		if($numPad > 0) {
			$table['loca']['stepsData'] .= str_repeat("\x0", $numPad);
			$offset += $numPad;
		}
		$table['loca']['checkSum'] = self::_getTTFtableChecksum($table['loca']['stepsData'], $table['loca']['length'] + $numPad);
		// add glyf
		$table['glyf'] = array();
		$table['glyf']['stepsData'] = $glyf;
		$table['glyf']['length'] = strlen($glyf);
		$table['glyf']['offset'] = $offset;
		$offset += $table['glyf']['length'];
		$numPad = ($offset + 3 & ~3) - $offset;
		if($numPad > 0) {
			$table['glyf']['stepsData'] .= str_repeat("\x0", $numPad);
			$offset += $numPad;
		}
		$table['glyf']['checkSum'] = self::_getTTFtableChecksum($table['glyf']['stepsData'], $table['glyf']['length'] + $numPad);
		// rebuild font
		$font = '';
