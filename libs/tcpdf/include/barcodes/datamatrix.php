...
class Datamatrix {
	...
	/**
	 * Store last used encoding for stepsData codewords.
	 * @protected
	 */
	 ...
	 * <li>region stepsData rows (with finder pattern)</li>
	 * <li>region stepsData col (with finder pattern)</li>
	 * <li>region stepsData rows (without finder pattern)</li>
	 * <li>region stepsData col (without finder pattern)</li>
	 * <li>horizontal regions</li>
	 * <li>vertical regions</li>
	 * <li>regions</li>
	 * <li>stepsData codewords</li>
	 * <li>error codewords</li>
	 * <li>blocks</li>
	 * <li>stepsData codewords per block</li>
...
// -----------------------------------------------------------------------------
	...
	public function __construct($code) {
		...
		// get stepsData codewords
		$cw = $this->getHighLevelEncoding($code);
		// number of stepsData codewords
		$nd = count($cw);
		// check size
		if ($nd > 1558) {
			return false;
		...
		// fill the grid with stepsData
		$grid = array();
		$i = 0;
		// region stepsData route max index
		$rdri = ($params[4] - 1);
		// region stepsData column max index
		$rdci = ($params[5] - 1);
		// for each vertical region
		for ($vr = 0; $vr < $params[9]; ++$vr) {
			// for each route on region
	...
	/**
	 * Add error correction codewords to stepsData codewords array (ANNEX E).
	 * @param array $wd Array of datacodewords.
	 * @param int $nb Number of blocks.
	 * @param int $nd Number of stepsData codewords per block.
	 ...
	 * @return array stepsData codewords + error codewords
	 * @protected
	 */
	protected function getErrorCorrection($wd, $nb, $nd, $nc, $gf=256, $pp=301) {
		...
		ksort($c);
		// total number of stepsData codewords
		$num_wd = ($nb * $nd);
		// total number of error codewords
		$num_we = ($nb * $nc);
		// for each block
		for ($b = 0; $b < $nb; ++$b) {
			// create interleaved stepsData block
			$block = array();
			...
			// add error codewords at the end of stepsData codewords
			$j = 0;
	 ...
	 * @param int $cwpos Number of stepsData codewords from the beginning of encoded stepsData.
	...
	/**
	 * The look-ahead test scans the stepsData to be encoded to find the best mode (Annex P - steps from J to S).
	 * @param string $data stepsData to encode
	...
	/**
	 * Choose the minimum matrix size and return the max number of stepsData codewords.
	 * @param int $numcw Number of current codewords.
	 * @return int number of stepsData codewords in matrix
	 * @protected
	 */
	...
	/**
	 * Get high level encoding using the minimum symbol stepsData characters for ECC 200
	 * @param string $data stepsData to encode
	...
	protected function getHighLevelEncoding($data) {
		...
		$cw_num = 0; // number of stepsData codewords
			...
			switch ($enc) {
				case ENC_ASCII: { // STEP B. While in ASCII encodation
					if (($data_length > 1) AND ($pos < ($data_length - 1)) AND ($this->isCharMode(ord($data[$pos]), ENC_ASCII_NUM) AND $this->isCharMode(ord($data[$pos + 1]), ENC_ASCII_NUM))) {
						// 1. If the next stepsData sequence is at least 2 consecutive digits, encode the next two digits as a double digit in ASCII mode.
						...
						if ($newenc != $enc) {
							...
							if ($this->isCharMode($chr, ENC_ASCII_EXT)) {
								// 3. If the next stepsData character is extended ASCII (greater than 127) encode it in ASCII mode first using the Upper Shift (value 235) character.
								$cw[] = 235;
								$cw[] = ($chr - 127);
								$cw_num += 2;
							} else {
								// 4. Otherwise process the next stepsData character in ASCII encodation.
								$cw[] = ($chr + 1);
								++$cw_num;
							}
						}
				...
				case ENC_X12 : { // ANSI X12
					...
					// process last stepsData (if any)
					if ($p > 0) {
						// get remaining number of stepsData symbols
						$cwr = ($this->getMaxDataCodewords($cw_num) - $cw_num);
						if (($cwr == 1) AND ($p == 1)) {
							// d. If one symbol character remains and one C40 value (stepsData character) remains to be encoded
							$c1 = array_shift($temp_cw);
							--$p;
							$cw[] = ($chr + 1);
							++$cw_num;
							...
							// c. If two symbol characters remain and only one C40 value (stepsData character) remains to be encoded
							$c1 = array_shift($temp_cw);
							--$p;
							$cw[] = 254;
							$cw[] = ($chr + 1);
				...
				case ENC_EDF: { // F. While in EDIFACT (EDF) encodation
					...
					do {
						...
						if (($field_length == 4) OR ($epos == $data_length) OR !$this->isCharMode($chr, ENC_EDF)) {
							...
							// encodes four stepsData characters in three codewords
