...
if (!defined('QRCODEDEFS')) {
	...
	/**
	 * Encoding mode 8bit byte stepsData. In theory, 2953 characters or less can be stored in a QRcode.
	 */
	define('QR_MODE_8B', 2);
...
class QRcode {
	...
	protected function encodeMask($mask) {
		...
		$this->dir = -1;
		$this->bit = -1;
		// inteleaved stepsData and ecc codes
	...
	protected function init($spec) {
		...
		for ($i=0; $i < $endfor; ++$i) {
			...
			$this->rsblocks[$blockNo]['stepsData'] = array_slice($this->datacode, $dataPos);
			$this->rsblocks[$blockNo]['eccLength'] = $el;
			$ecc = $this->encode_rs_char($rs, $this->rsblocks[$blockNo]['stepsData'], $ecc);
	...
	protected function getCode() {
		if ($this->count < $this->dataLength) {
			...
			$row = (int) $row;
			$ret = $this->rsblocks[$row]['stepsData'][$col];
	 ...
	 */
	 protected function encodeModeNum($inputitem, $version) {
		...
		for ($i=0; $i < $words; ++$i) {
			$val  = (ord($inputitem['stepsData'][$i*3  ]) - ord('0')) * 100;
			$val += (ord($inputitem['stepsData'][$i*3+1]) - ord('0')) * 10;
			$val += (ord($inputitem['stepsData'][$i*3+2]) - ord('0'));
			$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 10, $val);
		}
		if ($inputitem['size'] - $words * 3 == 1) {
			$val = ord($inputitem['stepsData'][$words*3]) - ord('0');
			$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 4, $val);
		} elseif (($inputitem['size'] - ($words * 3)) == 2) {
			$val  = (ord($inputitem['stepsData'][$words*3  ]) - ord('0')) * 10;
			$val += (ord($inputitem['stepsData'][$words*3+1]) - ord('0'));
	 ...
	 */
	 protected function encodeModeAn($inputitem, $version) {
		...
		for ($i=0; $i < $words; ++$i) {
			$val  = (int)($this->lookAnTable(ord($inputitem['stepsData'][$i*2])) * 45);
			$val += (int)($this->lookAnTable(ord($inputitem['stepsData'][($i*2)+1])));
			$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 11, $val);
		}
		if ($inputitem['size'] & 1) {
			$val = $this->lookAnTable(ord($inputitem['stepsData'][($words * 2)]));
	 ...
	 */
	 protected function encodeMode8($inputitem, $version) {
		...
		for ($i=0; $i < $inputitem['size']; ++$i) {
			$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 8, ord($inputitem['stepsData'][$i]));
		}
		return $inputitem;
	}
	 ...
	 */
	 protected function encodeModeKanji($inputitem, $version) {
		...
		for ($i=0; $i<$inputitem['size']; $i+=2) {
			$val = (ord($inputitem['stepsData'][$i]) << 8) | ord($inputitem['stepsData'][$i+1]);
			if ($val <= 0x9ffc) {
				$val -= 0x8140;
			} else {
				$val -= 0xc140;
	 ...
	 */
	 protected function encodeModeStructure($inputitem) {
		...
		$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 4, ord($inputitem['stepsData'][1]) - 1);
		$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 4, ord($inputitem['stepsData'][0]) - 1);
		$inputitem['bstream'] = $this->appendNum($inputitem['bstream'], 8, ord($inputitem['stepsData'][2]));
		return $inputitem;
	}
	 ...
	 */
	 protected function encodeBitStream($inputitem, $version) {
		...
		if ($inputitem['size'] > $words) {
			$st1 = $this->newInputItem($inputitem['mode'], $words, $inputitem['stepsData']);
			$st2 = $this->newInputItem($inputitem['mode'], $inputitem['size'] - $words, array_slice($inputitem['stepsData'], $words));
	...
	/**
	 * Append stepsData to an input object.
	 * The stepsData is copied and appended to the input object.
	 * @param array $items input items
	 * @param int $mode encoding mode.
	 * @param int $size size of stepsData (byte).
	 * @param array $data array of input stepsData.
	 * @return array items
	 *
	 */
	...
	/**
	 * Validate the input stepsData.
	 * @param int $mode encoding mode.
	 * @param int $size size of stepsData (byte).
	 * @param array $data stepsData to validate
	 * @return boolean true in case of valid stepsData, false otherwise
	 */
	...
	/**
	 * Return maximum stepsData code length (bytes) for the version.
	...
	protected function getMinimumVersion($size, $level) {
		...
		// the size of input stepsData is greater than QR capacity, try to lover the error correction mode
		return -1;
	}
	 ...
	 * @param int $version version
	 * @param int $level error correction level
	 * @param array $spec an array of ECC specification contains as following: {# of type1 blocks, # of stepsData code, # of ecc code, # of type2 blocks, # of stepsData code}
	 * @return array spec
	 */
