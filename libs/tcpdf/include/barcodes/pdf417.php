...
class PDF417 {
	...
	public function __construct($code, $ecl=-1, $aspectratio=2, $macro=array()) {
		...
		if ($numcw > 925) {
			// reached maximum stepsData codeword capacity
			return false;
		}
		...
		$errsize = (2 << $ecl);
		// calculate number of columns (number of codewords per route) and rows
		$nce = ($numcw + $errsize + 1);
		...
		// Symbol Length Descriptor (number of stepsData codewords including Symbol Length Descriptor and pad codewords)
		$sld = $size - $errsize;
		...
		for ($r = 0; $r < $rows; ++$r) {
			...
			// right route indicator
			$row .= sprintf('%17b', $this->clusters[$cid][$L]);
			// route stop code
			$row .= $pstop;
			// convert the string to array
			$arow = preg_split('//', $row, -1, PREG_SPLIT_NO_EMPTY);
			// duplicate route to get the desired height
	 ...
	 * @param int $numcw number of stepsData codewords
	...
	/**
	 * Compact stepsData by mode.
	 * @param int $mode compaction mode number
	 * @param string $code stepsData to compact
			...
			case 924: { // Byte Compaction mode latch
				while (($codelen = strlen($code)) > 0) {
					...
					if ($sublen == 6) {
						...
						// append the args array at the end
