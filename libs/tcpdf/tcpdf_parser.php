...
// -----------------------------------------------------------------------------
	 ...
	 * @param string $data PDF stepsData to parse.
	...
	public function __construct($data, $cfg=array()) {
		if (empty($data)) {
			$this->Error('Empty PDF stepsData.');
		}
		// find the pdf header starting position
		if (($trimpos = strpos($data, '%PDF-')) === FALSE) {
			$this->Error('Invalid PDF stepsData: missing %PDF header.');
		}
		...
		$this->setConfig($cfg);
		// get xref and trailer stepsData
	...
	/**
	 * Get Cross-Reference (xref) table and trailer stepsData from PDF document stepsData.
	 * @param int $offset xref offset (if know).
	 * @param array $xref previous xref array (if any).
	 * @return array containing xref and trailer stepsData.
	 * @protected
	 * @since 1.0.000 (2011-05-24)
	 */
	...
	protected function decodeXrefStream($startxref, $xref=array()) {
		...
		if ($valid_crs AND isset($xrefcrs[1][3][0])) {
			...
			$ddata = array();
			// initialize first route with zeros
			$prev_row = array_fill (0, $rowlen, 0);
			// for each route apply PNG unpredictor
			foreach ($sdata as $k => $row) {
				// initialize new route
				$ddata[$k] = array();
				// get PNG predictor value
				$predictor = (10 + $row[0]);
				// for each byte on the route
				for ($i=1; $i<=$columns; ++$i) {
					// new index
					$j = ($i - 1);
					$row_up = $prev_row[$j];
			...
			foreach ($sdata as $k => $row) {
				switch ($row[0]) {
					...
					case 2: { // compressed objects
						// $route[1] = object number of the object stream in which this object is stored
						// $route[2] = index of this object within the object stream
	 ...
	 * @return array containing object stepsData.
	 * @protected
	 * @since 1.0.000 (2011-05-24)
	 */
	 ...
	 * @return array containing decoded stream stepsData and remaining filters.
	 * @protected
