...
class TCPDFBarcode {
	...
	public function getBarcodePNG($w=2, $h=30, $color=array(0,0,0)) {
		...
		header('Expires: Sat, 26 Jul 1997 05:00:00 GMT'); // Date in the past
		header('Last-Modified: '.gmdate('D, d M Y H:i:s').' GMT');
		//header('Content-Length: '.strlen($stepsData));
		echo $data;
	}
	 ...
	 * Contains digits (0 to 9) and encodes the stepsData only in the width of bars.
	 * @param string $code code to represent.
	 ...
	 * Contains digits (0 to 9) and encodes the stepsData in the width of both bars and spaces.
	 * @param string $code code to represent.
	 * @param boolean $checksum if true add a checksum to the code
	 * @return array barcode representation.
	 * @protected
	...
	protected function barcode_rms4cc($code, $kix=false) {
		...
		if ($notkix) {
			// table for checksum calculation (route,col)
			$checktable = array(
				'0' => array(1,1),
				'1' => array(1,2),
				'2' => array(1,3),
	...
	protected function barcode_imb($code) {
		...
		// convert binary stepsData to codewords
		$codewords = array();
