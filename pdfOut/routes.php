...
if (isset($_GET['pid']) && isset($_GET['orid'])) {
    ...
    $cellHeight = 7; // Начальная высота ячейки
    ...
    if ($assy_progress) {
        ...
        foreach ($assy_progress as $step) {
            ...
            if ($step['serial_number'] != '0') {
                $pdf->Cell(155, $cellHeight, $step['serial_number'], 1, 0, 'C');
                $pdf->Cell(25, $cellHeight, 'Serial', 1, 1, 'C');
            }
            // assy step progress data
            $data = json_decode($step['route_card_body']);
            // таблица рут актов получаем данный рут акт
            $route = R::load(ROUTE_ACTION, $data->route_id);
            $pdf->checkAndAddNewPage($cellHeight); // Проверяем, нужна ли новая страница
            // формируем строку на странице
            ...
            $pdf->addCellWithAutoFont(20, $cellHeight, "$date\n$time", 1, 0, 'C', $maxChar); // время окончания работ
            ...
            $pdf->addCellWithAutoFont(65, $cellHeight, $route['actions'], 1, 0, 'C', $maxChar); // описания действий
